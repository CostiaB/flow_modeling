def main(N_it, p_it, freq, stop_it):
    ...
    c0 = params_conc.c0
    concentration = params_conc.concentration
    ca = params_conc.ca
    c = params_conc.c
    ...
    try:
        ...
        print(f'Freq: {freq}, N_it: {N_it}, p_it: {p_it}')
        ...
        for a in tqdm(range(N_it)):
            u, v, p = navier_stokes(u, v, freq,
                              ...
                              stepcount=a, save_vectors=False, silent=True)
        ...
        for img in imgs:
            img[p_gif!=border_value+10000] -= concentration
        np.save('imgs', imgs )
        vmin = np.mean(imgs[imgs<0])
        vmax = -1 * vmin
        ...
        write_params(save_folder, 
                     p_it=p_it, N_it=N_it, freq=freq, stop_it=stop_it)    
...
if __name__ == '__main__':
    ...
    main(N_it=args.N_it, p_it=args.p_it, freq=args.freq, stop_it=args.stop_it)